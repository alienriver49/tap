...
@inject('TapFx')
export class ExtensionLoaderEngine {
    constructor(
    ...
    private _getExtensionBundle(extensionName: string): string {
        return 'src/exts/' + extensionName + '/bootstrap';
    }
    ...
    public loadExtension(extensionId: string, extensionName: string): Promise<string> {
        return new Promise<string>((resolve) => {
            const extScript = this._getExtensionBundle(extensionName);
            const extensionScripts: IScript[] = [
            ...
            const bootstrapScripts = (scripts: IScript[]) => {
            ...
            const onIFrameLoad: EventListener = (e: Event): void => {
            ...
            const iFramesEl = window.document.getElementById('extension-iframes');
            if (iFramesEl) {
    ...
    public unloadExtension(extensionId: string): void {
        ...
        const iFrameElement = document.getElementById(extensionId);
        if (iFrameElement) {
